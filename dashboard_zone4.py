...
def show_dashboard_zone4():
    st.title("Zone 4 EDA Dashboard")
    st.write("This is the dashboard for EDA of Zone 4.")
    ...
    for feature in numerical_features:
        ...
        # Histogram
        st.plotly_chart(px.histogram(zone_4_df, x=feature, marginal="box", color_discrete_sequence=px.colors.qualitative.Set3))
        
        # Scatter Plot
        st.plotly_chart(px.scatter(zone_4_df, x='Latitude', y='Longitude', color=feature, color_continuous_scale=px.colors.diverging.Tealrose))
        
        # Line Plot
        st.plotly_chart(px.line(zone_4_df.sort_values(by='Latitude'), x='Latitude', y=feature, color_discrete_sequence=px.colors.qualitative.Set3))
        
        # Bar Chart
        st.plotly_chart(px.bar(zone_4_df, x='Latitude', y=feature, color_discrete_sequence=px.colors.qualitative.Set3))
        
        # Violin Plot
        st.plotly_chart(px.violin(zone_4_df, y=feature, color_discrete_sequence=px.colors.qualitative.Set3))
