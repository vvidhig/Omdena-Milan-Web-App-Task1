...
# Set page config
st.set_page_config(layout="wide", page_title="Zone 9 Dashboard")
...
st.write(zone_9_df, unsafe_allow_html=True)
...
for feature in numerical_features:
    ...
    with col1:
        ...
        fig = px.histogram(zone_9_df, x=feature, marginal="box", color_discrete_sequence=[px.colors.qualitative.Set3[0]])
        st.plotly_chart(fig, use_container_width=True)

    # Box Plot
    with col2:
        st.write(f"Box Plot of {feature}")
        fig = px.box(zone_9_df, y=feature, color_discrete_sequence=[px.colors.qualitative.Set3[1]])
        st.plotly_chart(fig, use_container_width=True)

    # Scatter Plot
    with col3:
        st.write(f"Scatter Plot of {feature}")
        fig = px.scatter(zone_9_df, x='Latitude', y='Longitude', color=feature, color_continuous_scale=px.colors.diverging.Tealrose)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col1:
        ...
        fig = px.line(zone_9_df.sort_values(by='Latitude'), x='Latitude', y=feature, color_discrete_sequence=[px.colors.qualitative.Set3[2]])
        st.plotly_chart(fig, use_container_width=True)

    # Bar Chart
    with col2:
        st.write(f"Bar Chart of {feature}")
        fig = px.bar(zone_9_df, x='Latitude', y=feature, color_discrete_sequence=[px.colors.qualitative.Set3[3]])
        st.plotly_chart(fig, use_container_width=True)

    # Violin Plot
    with col3:
        st.write(f"Violin Plot of {feature}")
        fig = px.violin(zone_9_df, y=feature, color_discrete_sequence=[px.colors.qualitative.Set3[4]])
        st.plotly_chart(fig, use_container_width=True)
...
fig = px.scatter_3d(zone_9_df, x='SMI', y='NDVI', z='LST', color='solar_radiation', color_continuous_scale=px.colors.diverging.Tealrose)
st.plotly_chart(fig, use_container_width=True)
